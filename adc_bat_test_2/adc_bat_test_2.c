...
void getCellVoltages(double adc_cell_data[][6], int sampleNum);
...
int main(void) {
    ...
    // Keep track of last 30 cell voltage values
    double adc_cell_data[30][6];
    ...
    int j = 0;
    for (j=0; j<30; j++){
    	getCellVoltages(adc_cell_data,j);
    	__delay_cycles(1000);
    }
...
void getCellVoltages(double adc_cell_data[][6], int sampleNum){
	...
	double multiple_cell_voltages[6];
	...
	for (i=0; i<5; i++){
	    adc_cell_data[sampleNum][i] = multiple_cell_voltages[i]-multiple_cell_voltages[i+1];
	}
	adc_cell_data[sampleNum][5] = multiple_cell_voltages[5];
